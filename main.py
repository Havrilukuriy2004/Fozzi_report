...
def filter_data(df, week, report_type):
    if report_type == 'со счетом':
        ...
        df_filtered = df_filtered[~df_filtered['Получатель'].str.contains('|'.join(mask_keywords), case=False, na=False)]
        df_filtered = df_filtered[~df_filtered['Получатель'].str.contains('район', case=False, na=False) | df_filtered[
            'Получатель'].str.contains('крайон', case=False, na=False)]
    return df_filtered
...
def create_dashboard(df):
    ...
    top_payments = filtered_data.groupby('Получатель')['Сумма'].sum().nlargest(10).reset_index()
...
def main():
    ...
    if file_path:
        ...
        if os.path.exists(file_path):
            ...
            # Create the dashboard with the loaded data
            create_dashboard(df)
        else:
