...
def create_dashboard(df):
    ...
    if not filtered_data.empty:
        ...
        def style_func(data):
            data = data.style.applymap(highlight_values)
            data = data.applymap(highlight_first_last, subset=pd.IndexSlice[:, [0, -1]])
            data = data.applymap(highlight_first_last, subset=pd.IndexSlice[[0, -1], :])
            return data

        st.table(style_func(recipients_pivot.format("{:,.0f}")))
    ...
    if not filtered_data.empty:
        ...
        st.table(style_func(top_recipients.rename(
            columns={'code': 'ЄДРПОУ отримувача', 'recipient': 'Отримувач', 'sum': 'Сума, тис. грн.'}).format(
            {'Сума, тис. грн.': '{:,.0f}'})))
    ...
    if not filtered_data.empty:
        ...
        st.table(style_func(summary_df))
    ...
    if not filtered_data.empty:
        ...
        st.table(
            style_func(supplier_totals.rename(columns={'payer': 'Платник', 'sum': 'Сума, тис. грн'}).format({'Сума': '{:,.0f}'})))
    else:
        st.write("Нет данных для выбранных фильтров.")
